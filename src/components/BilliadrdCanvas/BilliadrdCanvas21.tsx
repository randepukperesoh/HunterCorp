...
export const BilliadrdCanvas: React.FC<CanvasProps> = ({settings}) => {
  ...
  let balls :Ball[] = [
    { x: 50, y: 50, radius: 10, color: 'red', speed:0, corner: 0, picked: false },
    { x: 150, y: 150, radius: 15, color: 'green', speed:0, corner: 0, picked: false},
  ];
...
const addBall = () => {
  balls.push({ x: Math.floor(Math.random() * settings.width ), y: Math.floor(Math.random() * settings.height ), radius: 5 + Math.floor(Math.random() * 10 ), color: 'red', speed:0, corner: 0, picked: false })
}
  return (
    <div>
      ...
      />
      {balls.map((ball, id) => {
        return ball.picked && <div key={ball.color + id}>{ball.color}</div> 
      })}
    </div>
  );
};
